...
void loop() {
  ...
  if (Tr1State == "empty" && Tr2State == "empty" && Tr3State == "empty" && Tr4State == "empty" && !firstRecording){
    for (int j = 0; j < qtyLEDs - nonRingLEDs; j++){
      LEDs[j] = CRGB(0, 0, 0);
      FastLED.show();
    }
    sendNote(0x1F);
  ...
  if ((Tr1State == "muted" || Tr1State == "empty") && (Tr2State == "muted" || Tr2State == "empty") && (Tr3State == "muted" || Tr3State == "empty") && (Tr4State == "muted" || Tr4State == "empty") && !firstRecording) stopMode = true;
  else stopMode = false;
...
void setLEDs(){
  ...
  else if (Tr1State == "muted" && Tr2State == "muted" && Tr3State == "muted" && Tr4State == "muted" || stopMode) setHue = 160;  //set the led ring to blue when all tracks are muted
  else setHue = 96;
